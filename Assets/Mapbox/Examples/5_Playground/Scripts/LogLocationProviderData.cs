namespace Mapbox.Examples.Scripts
{
    ...
    public class LogLocationProviderData : MonoBehaviour
    {
        ...
        [SerializeField]
        private TMP_InputField _locText;
		...
		void LocationProvider_OnLocationUpdated(Location location)
		{
            ...
            Vector2d coordinates = location.LatitudeLongitude;

            // Call the GetLocationName method to retrieve the location name
        ...
        private void GetLocationName(Vector2d coordinates)
        {
            string query = $"{coordinates.x},{coordinates.y}";
            GeocodeForward(query);
        }

        private void GeocodeForward(string query)
        {
            ForwardGeocodeResource forwardGeocodeResource = new ForwardGeocodeResource(query);
            Geocoder geocoder = MapboxAccess.Instance.Geocoder;
            geocoder.Geocode(forwardGeocodeResource, OnGeocodingResponse);
        }

        private void OnGeocodingResponse(ForwardGeocodeResponse res)
        {
            if (res != null && res.Features.Count > 0)
            {
                string locationName = res.Features[0].PlaceName;
                _locText.text = locationName;
            }
            else
            {
                _locText.text = "No location name found for the given coordinates.";
            }
        }
